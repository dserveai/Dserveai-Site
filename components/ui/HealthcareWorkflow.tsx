...
import styles from "./HealthcareWorkflow.module.css";
...
export default function HealthcareWorkflow({ steps, color }: Props) {
    ...
    <div className={styles.container} style={{ "--c": color } as React.CSSProperties}>
      ...
      <div className={styles.centralMap}>
        <div className={`${styles.mapCore} ${hoveredIdx !== null ? styles[`mode-${hoveredIdx}`] : ''}`}>
          ...
          {/* Animated ECG Line (Appears Mode 1) */}
          <svg className={styles.ecgLine} viewBox="0 0 320 100">
            <path className={styles.ecgPath} d="M 0 50 L 80 50 L 100 20 L 120 90 L 140 10 L 160 70 L 180 50 L 320 50" />
          </svg>

          {/* Central Bio Core */}
          <div className={styles.bioCore}>
            <Activity size={32} />
          </div>

          {/* Vitals Tags */}
          <div className={`${styles.vitalsTag} ${styles.tagBpm}`}>[BPM: 72]</div>
          <div className={`${styles.vitalsTag} ${styles.tagO2}`}>[O2: 99%]</div>
          <div className={`${styles.vitalsTag} ${styles.tagDiag}`}>[Diagnosis: Normal]</div>
