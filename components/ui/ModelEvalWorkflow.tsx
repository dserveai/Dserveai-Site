...
import { Cpu, ThumbsUp, ThumbsDown, Star } from "lucide-react";
import styles from "./ModelEvalWorkflow.module.css";
...
export default function ModelEvalWorkflow({ steps, color }: Props) {
    ...
    <div className={styles.container} style={{ "--c": color } as React.CSSProperties}>
      ...
      <div className={styles.centralMap}>
        <div className={`${styles.mapCore} ${hoveredIdx !== null ? styles[`mode-${hoveredIdx}`] : ''}`}>
          ...
          <svg className={styles.confidenceRing} viewBox="0 0 100 100">
            <circle className={styles.ringBg} cx="50" cy="50" r="45" />
            <circle className={styles.ringFill} cx="50" cy="50" r="45" pathLength="800" />
          </svg>

          {/* AI Processing Core */}
          <div className={styles.aiCore}>
            <Cpu size={40} className={styles.coreIcon} />
            <div className={styles.coreScore}>99%</div>
          </div>

          {/* Floating Human Feedback Nodes */}
          <div className={styles.feedbackNodes}>
            <div className={`${styles.node} ${styles.nodeUp}`}><ThumbsUp size={16} /></div>
            <div className={`${styles.node} ${styles.nodeDown}`}><ThumbsDown size={16} /></div>
            <div className={`${styles.node} ${styles.nodeStar}`}><Star size={16} /></div>
            
            {/* Reward Signal Particles */}
            <div className={`${styles.rewardSignal} ${styles.signal1}`} />
            <div className={`${styles.rewardSignal} ${styles.signal2}`} />
          </div>
